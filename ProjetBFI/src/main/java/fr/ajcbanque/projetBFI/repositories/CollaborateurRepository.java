...
public class CollaborateurRepository implements ICollaborateurRepository {
	...
	Query query = em.createQuery(
		"select new fr.ajcbanque.projetBFI.dto.CollaborateurDTO(c.id, c.email, c.matriculeInterne, c.nom, c.prenom)"
			+ "from Collaborateur c" + "where c.id=" + id);
	return (CollaborateurDTO) query.getSingleResult();
    }
}
